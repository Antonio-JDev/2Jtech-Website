...
const CELL_SIZE = 72; // Desktop-ish size. Will dictate cols/rows
const INFLUENCE_RADIUS = 260;
const MAX_WARP = 24;
const DOT_SPACING = 32;
const LERP_SPEED = 0.42;
...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      // default → brand neon (#00EFFC); monochrome → white
      const theme = {
        default: {
          bg: "#050505",
          lineActive: { r: 0, g: 239, b: 252, a: 0.9 },
          nodeActive: { r: 0, g: 239, b: 252, a: 1.0 },
          glow: "0,239,252",
          ripple: "0,239,252",
        },
        monochrome: {
  ...
  const animate = useCallback(
    (now: number) => {
      ...
      // First real pointer sample: snap instead of lerping from off-canvas
      if (m.x < -9000 && t.x > -9000) {
        m.x = t.x;
        m.y = t.y;
        draw(now);
  ...
  return (
    <div
      className={cn(
        "relative w-full min-h-screen overflow-x-clip",
        globalColor === "monochrome" ? "bg-[#000000]" : "bg-[#050505]",
        className,
      )}
    >
      <canvas
